...
const AssetDetails = ({
  ...
  const consume = async (consumeAsset: DDO) => {
    ...
    if (accounts) {
      const agreement = await instance?.assets.order(
        consumeAsset.id,
        accounts[0]
      );
      console.log("agreement----", agreement);
      if (agreement) {
        await instance?.assets.consume(
          agreement,
          consumeAsset.id,
          accounts[0],
          "",
  ...
  return (
    <Card variant="outlined" className={classes.card} raised={true}>
      <CardHeader
        avatar={
      ...
      <CardContent>
        <Typography>
          Rank score: {Math.round((rank + Number.EPSILON) * 100) / 100}{" "}
        </Typography>
        <Typography className={classes.title} color="textSecondary">
          Price:{" "}
          {
            assetInfo.service.find((e) => e.type === "metadata")?.attributes
              .main.price
          }
        </Typography>
        <Typography className={classes.title} color="textSecondary">
