...
const setAssetReducer = (
  state: AssetListState,
  { payload }: PayloadAction<DDO[]>
) => {
  state.assets = payload;
};

const setStakesReducer = (
  state: AssetListState,
  { payload }: PayloadAction<StakeInterFaceMap>
) => {
  state.stakes = payload;
};

const setRanksReducer = (
  state: AssetListState,
  { payload }: PayloadAction<RanksInterface>
) => {
  state.ranks = payload;
};

const setMyStakesReducer = (
  state: AssetListState,
  { payload }: PayloadAction<MyStakeInterfaceMap>
) => {
...
export const setAssetListInfo = (
  ...
  const keysSorted: string[] = Object.keys(res)
    .sort((a, b) => {
      return res[a] - res[b];
    })
    .reverse();
